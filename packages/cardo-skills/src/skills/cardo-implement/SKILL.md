...
description: >
  ...
  Do NOT use for planning (cardo-plan) or reviewing changes (cardo-review /
  cardo-simplify).
---

# Cardo Implement — PBT-first execution against an explicit requirements list

Pipeline position: after `cardo-plan`, or standalone when the task is
...
1. Define the business logic as invariants (same phrasing discipline as
   `cardo-pbt-debugging`: total functions, round-trips, idempotence, no
   loss/no duplication).
...
4. Follow project conventions (`AGENTS.md`): lint / typecheck / build, tests
   for new behaviour. Leave changes UNCOMMITTED — `cardo-review` /
   `cardo-simplify` read the diff.
