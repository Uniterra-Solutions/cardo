/**
 * Copy dictionaries for the Cardo settings section. Product copy is Chinese
...
export const zh = {
  nav: 'Cardo',
  intro:
    '配置 Cardo 网关：API 密钥、网关地址、接口协议与模型列表。模型发现只列出支持 chat 接口的模型。',
  keyInput: 'API 密钥',
  ...
  saved: '已保存。',
  loadFailed: '加载失败',
  nsNotRegistered: 'llm-cardo: 设置命名空间未注册（插件行是否已加载？）',
  retry: '重试',
  readOnly: '当前设置源只读，无法保存。',
  modelHint:
    '默认只列出 /chat/completions 接口支持的模型；不支持该接口的模型请手动添加。模型可单独选择 responses 协议。该配置可在 settings.yaml 的 llm-cardo: 段用 modelExcludePatterns 调整。',
} as const;

export type CardoKey = keyof typeof zh;

export const en: Record<CardoKey, string> = {
  nav: 'Cardo',
  intro:
    'Configure the Cardo gateway: API key, gateway base URL, wire protocol, and model list. Discovery lists chat-capable models only.',
  keyInput: 'API key',
  ...
  nsNotRegistered:
    'llm-cardo: the settings namespace is not registered (is the plugin row loaded?)',
  retry: 'Retry',
  readOnly: 'The active settings source is read-only; nothing can be saved.',
  modelHint:
    'By default only models supported by the /chat/completions endpoint are listed; models without that support must be added manually. Models may select the responses protocol individually. Tune modelExcludePatterns in the llm-cardo: settings section.',
};
