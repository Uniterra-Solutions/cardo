/**
 * The Cardo settings section: API key (write-only), gateway base URL, and
 ...
 * from the fiber-scoped `cardo-*` stylesheet the apply closure injects; it
...
export interface CardoSectionProps {
  api: Pick<IApiClient, 'settings' | 'credentials' | 'llm'>;
  t: (key: CardoKey) => string;
...
/**
 * Render the Cardo settings section.
 * @param props - the wire face and the bound translate.
 * @returns the section.
 */
export function CardoSection(props: CardoSectionProps): ReactNode {
  const { api, t } = props;
  ...
  if (status === 'error') {
    return (
      <section aria-label={t('nav')}>
        <p className="cardo-error">{`${t('loadFailed')}: ${errorText ?? ''}`}</p>
        <button
          type="button"
          className="cardo-button"
          onClick={() => {
            void load();
          }}
        >
  ...
  return (
    <section aria-label={t('nav')}>
      ...
      {errorText === undefined ? null : <p className="cardo-error">{errorText}</p>}

      <div className="cardo-field">
        <label htmlFor="cardo-key">{t('keyInput')}</label>
      ...
      <div className="cardo-field">
        <label htmlFor="cardo-base">{t('baseUrl')}</label>
        <input
          id="cardo-base"
          type="text"
          className="cardo-input"
      ...
      <div className="cardo-field">
        <label htmlFor="cardo-protocol">{t('apiProtocol')}</label>
        <select
          id="cardo-protocol"
          className="cardo-select"
          value={protocol}
          onChange={(event) => {
        ...
        <p className="cardo-hint">{t('apiHint')}</p>
      </div>

      <section className="cardo-catalog" aria-label={t('models')}>
        <div className="cardo-catalog-head">
          <span className="cardo-catalog-title">{t('models')}</span>
          <div className="cardo-catalog-actions" style={{ display: 'flex', gap: 4 }}>
            <button
              type="button"
              className="cardo-linkbutton"
              disabled={busy}
              onClick={() => {
                void fetchModels();
              }}
        ...
        {models.length === 0 ? <p className="cardo-empty">{t('modelsEmpty')}</p> : null}
        {models.map((model, index) => (
          <div key={index} className="cardo-entry">
            <div className="cardo-modelrow">
              <input
                className="cardo-input"
                type="text"
              ...
              <button
                type="button"
                className="cardo-iconbutton cardo-iconbutton--danger"
            ...
            {expanded.has(index) ? (
              <div className="cardo-modeladvanced">
                <label className="cardo-modelfield">
                  <span className="cardo-modelfield-label">{t('contextWindow')}</span>
                  <input
                    className="cardo-input"
                    type="text"
                    inputMode="numeric"
                ...
                <label className="cardo-modelfield">
                  <span className="cardo-modelfield-label">{t('maxTokens')}</span>
                  <input
                    className="cardo-input"
                    type="text"
                    inputMode="numeric"
                ...
                <label className="cardo-modelfield">
                  <span className="cardo-modelfield-label">{t('modelApi')}</span>
                  <select
                    className="cardo-select"
                ...
                model.reasoningEfforts.length > 0 ? (
                  <label className="cardo-modelfield">
                    <span className="cardo-modelfield-label">{t('modelReasoning')}</span>
                    <select
                      className="cardo-select"
      ...
      {candidates === undefined ? null : (
        <div className="cardo-candidates">
          ...
          <button
            type="button"
            className="cardo-button cardo-button--primary"
      ...
      {params === undefined ? null : (
        <div className="cardo-params" ref={paramsRef}>
          <strong>{t('paramsTitle')}</strong>
          <p className="cardo-params-summary">
            {t('paramsSummary')
              .replace(
                '{matched}',
          ...
          {params.models.map((entry) => {
            if (entry.matches.length === 0) {
              return (
                <div key={entry.id} className="cardo-params-row">
                  <span className="cardo-params-id">{entry.id}</span>
                  <span className="cardo-params-unmatched">{t('paramsUnmatched')}</span>
                  <span />
                </div>
              );
            }
            if (entry.matches.length === 1) {
              ...
              return (
                <div key={entry.id} className="cardo-params-row">
                  <span className="cardo-params-id">{entry.id}</span>
                  <span className="cardo-params-values">
            ...
            return (
              <div key={entry.id} className="cardo-params-row">
                ...
                <span className="cardo-params-values">{match.provider}</span>
              </div>
            );
          })}
      ...
      <p className="cardo-hint">{t('modelHint')}</p>
