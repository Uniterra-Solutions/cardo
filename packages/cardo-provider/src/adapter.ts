/**
 * The cardo dual-protocol gateway adapter: OpenAI Chat Completions AND
 ...
 *
 * @module @cardo/cardo-provider/adapter
 */
...
export const PKG = 'cardo-provider';
...
export interface CardoCatalogModel {
...
export interface CardoConnectionOptions {
  ...
  models: readonly CardoCatalogModel[];
...
/** Constructor options for {@link CardoAdapter}. */
export interface CardoAdapterOptions {
  /** Current validated connection facts; called once per operation. */
  options: () => CardoConnectionOptions;
  /** Resolve the bearer token for one request's connection facts. */
  resolveApiKey: (connection: CardoConnectionOptions) => Promise<string>;
...
function modelInfo(provider: string, model: CardoCatalogModel): LlmModelInfo {
  return {
    provider,
    id: model.id,
...
function protocolOf(connection: CardoConnectionOptions, model: string): GatewayApi {
  return connection.models.find((entry) => entry.id === model)?.api ?? connection.api;
}

/**
 * The cardo gateway adapter. One instance serves every model name it was
 * registered under, routing each call to Chat Completions or Responses by the
 * model's pinned (or default) protocol.
 */
export class CardoAdapter extends LlmAdapter {
  constructor(private readonly config: CardoAdapterOptions) {
    super();
  }

  override providerInfo(provider: string): LlmProviderInfo {
    return { id: provider, name: 'Cardo Gateway' };
  }
  ...
  private async *request(
    ...
    signal: AbortSignal,
    connection: CardoConnectionOptions,
    apiKey: string,
    onComment: () => void,
