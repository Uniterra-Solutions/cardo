...
- **Chat Completions send back**: every assistant message with a reasoning block carries `reasoning_content` verbatim (even empty), and once any assistant message carried reasoning, every later tool-call turn carries the field too — a turn the model answered without reasoning round-trips as the empty marker (`reasoning_content: ""`). DeepSeek's thinking mode is all-or-nothing: a tool-call turn missing the field makes the next request fail with "The `reasoning_content` in the thinking mode must be passed back to the API"
