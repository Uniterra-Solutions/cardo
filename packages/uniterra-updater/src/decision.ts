/**
 * Uniterra update-check decision logic. Pure (no Electron, no fs) so the
 * startup-update prompt semantics are unit-testable. Ported from the old
 * pi-gui `uniterra-update-logic.ts` with the npm package renamed.
 */
...
export interface UniterraUpdateVersions {
  /** Version of the installed desktop app (app.getVersion()). */
  readonly appVersion: string;
  /** Version of the installed CLI (probed via `uniterra --version`), if known. */
  readonly cliVersion: string | undefined;
  /** Newest published app version (uniterra GitHub release tag), if known. */
...
export type UniterraUpdateResult =
  | {
...
export type UniterraUpdateAction =
  | { readonly action: 'quit-and-update' }
  | { readonly action: 'skip-version'; readonly skippedVersion: string }
  | { readonly action: 'none' };

/**
 * Map the update prompt's user response onto the action to take. `uniterra
...
export function resolveUpdateAction(
  result: UniterraUpdateResult,
  response: number,
): UniterraUpdateAction {
 ...
 * @uniterra-solutions/uniterra@latest update` — ALWAYS execute the latest
 ...
 * is it an error.
 */
export function resolveUniterraUpdateStatus(
  versions: UniterraUpdateVersions,
): UniterraUpdateResult {
  ...
  if (latestVersion === undefined) {
    return { status: 'error', message: 'Unable to determine the latest Uniterra version.' };
  }
