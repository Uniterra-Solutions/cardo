...
whether to load a skill. Never summarise the workflow — describe _what_ the
skill does and _when to use it_, not _how it works_.
...
Present the frontmatter to yourself: _"Based on this name + description,
generate 3 prompts that SHOULD trigger this skill and 3 that should NOT.
Critique the description and propose revisions if needed."_
...
Ask: _"What if a referenced file is missing? Input violates assumptions?
Script fails mid-execution?"_ Fix gaps found.
...
Example: _"The script was extracted because JSON parsing is fragile when
