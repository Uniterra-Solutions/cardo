...
`uniterra-implement`'s job (the design doc's PBT plan tells it which invariants
to encode).
...
- For each task, copy the FRs it satisfies into its `requirements` list as
  ...
  several.
- Choose `execution_mode` by task overlap (same rule uniterra-implement applies
  when executing):
...
`uniterra-implement`; if they ask for changes, revise and re-present.
...
- requirements: non-empty, self-contained per task; the acceptance list
  `uniterra-implement` writes failing property tests against.
