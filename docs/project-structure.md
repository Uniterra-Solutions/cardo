...
| `uniterra-review`        | Adversarial review: goal + context → review (correctness + security; pass/fail verdict) → repro (one agent pins all findings as failing regression tests) → fix → re-review loop until pass |
