...
**Purpose:** pi extension entry — registers the 6 pipeline tools and the 3 lifecycle events that drive the Jovaltus pipeline.

Source: `packages/jovaltus/src/index.ts` (662 LOC). Ported from the Hermes plugin's `src/jovaltus/tools.py` + `src/jovaltus/hooks.py`.
...
| `resume_session` | `{ session_id: string }` (required)        | Re-activates an `interrupted`/`failed` session (id or run_dir); re-dispatches from the resume target phase.  |
...
| -------------------- | ------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `before_agent_start` | `pre_llm_call`      | Injects `[Jovaltus pipeline] id=... tool=... phase=... status=...` into every main-agent turn's system prompt when a pipeline exists.                                                        |
...
- Outbound: `state.ts` (SQLite session machine), `chain.ts` (CHAIN/verdict), `dispatch.ts` (child runner), `prompts.ts` (render), `@earendil-works/pi-coding-agent` (types + ExtensionAPI), `typebox` (schemas), `node:fs`, `node:path`.
...
- **Resume target:** a `*_waiting` session resumes by falling back to the reviewer phase; any other session resumes at its stored phase, with a resume note appended to the child prompt (artifact-aware continuation).
