...
| dsh CLI (`dshCliPath`)         | `packages/cardo-desktop/node_modules/@deepseek-ai/dsh/lib/bin.js` | same, inside the embedded source (`Contents/Resources/src` on macOS, `resources/src` on Windows — both `process.resourcesPath`) — ALWAYS the desktop package's node_modules, never workspace root, never `vendor/dsh-runtime` |
