...
2. `removeRetiredBuiltins()` — strip retired built-ins' bundle rows, dependency entries, and `node_modules` copies (idempotent; the only pass that can remove them, since step 3 would otherwise early-return on an already-full profile).
3. No-op when `hasAllBuiltins` AND no vendored/workspace copy is stale.
4. Write `pnpm-workspace.yaml` (allowBuilds for native deps: node-pty, sharp, protobufjs, fsevents, tesseract.js; `minimumReleaseAge: 0`).
5. `dsh plugin add` each npm spec (env: `DSH_HOME`, `ELECTRON_RUN_AS_NODE=1`).
6. Copy each vendored + workspace built-in into `node_modules/<pkg-name>` and append its bundle row to the profile `package.json` `dsh.profile.bundles`.
7. Expected bundle rows: `@deepseek-ai/dsh-base`, `@deepseek-ai/dsh-web-app`, + every built-in package name.
