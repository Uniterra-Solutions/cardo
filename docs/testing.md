...
No test framework is installed (no vitest/jest; not in `package.json`). The repo gates are static only: `typecheck`, `lint`, `format:check`, `build`. There is no automated test suite yet.
...
`plan` a small requirement in a `pi -e packages/jovaltus/src/index.ts` session; confirm artifacts + verdict flow. Requires a configured model provider.
