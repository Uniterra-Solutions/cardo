...
| windows-install       | `scripts/verify-windows-install/verify.ps1`                    | real `uniterra setup --source` on windows-latest: package → embed → install → shortcut → `Uniterra.exe` boot smoke (runs in the release gate, not locally on macOS) |
...
| `packages/uniterra-provider/test/reasoning-preservation.test.mjs` | No loss / no duplication of reasoning, text, and tool calls across every gateway wire shape — per-shape regressions + seeded randomized interleavings (300 runs)                                                                                                                                                                                                                                                                                              |
