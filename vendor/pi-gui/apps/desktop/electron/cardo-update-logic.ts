/**
 * Cardo update-check decision logic. Pure (no electron imports) so the
 * startup-update prompt semantics are property-tested in the desktop PBT lane.
 */
...
export type CardoUpdateResult =
  | { readonly status: "update-available"; readonly latestVersion: string; readonly currentVersion: string }
  | { readonly status: "up-to-date"; readonly currentVersion: string }
  | { readonly status: "error"; readonly message: string };

/**
 * Combine the CLI and GitHub-release checks into one verdict. An update exists
 * when either published version is newer than its installed counterpart; the
 * reported latestVersion is the newest of the two (used as the skip key).
 * Unknown lookups are ignored — only when BOTH lookups fail is it an error.
 */
 ...
 * we never claim an update we can't verify. Cardo-owned copy (the vendored
 * update-checker.ts keeps the upstream one for subtree-merge cleanliness).
 */
