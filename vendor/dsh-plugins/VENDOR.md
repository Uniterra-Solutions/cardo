...
| `dsh-git-graph` | `1841220388zzzcccxxx-star/dsh-git-graph` | `6b9899060ae28decfad065d9656519591f6013d7` | Embedded git repo graph visualizer: commit DAG, branch filter, file diff, VSCode-style uncommitted changes. Zero npm dependencies (server half uses node builtins + `git` binary); serves its own `web/index.html` relative to the package root, so the whole `web/` dir ships. Patch is a root `insert` of one loader row. |
...
dsh plugin --profile cardo add /absolute/path/vendor/dsh-plugins/dsh-git-graph
```
