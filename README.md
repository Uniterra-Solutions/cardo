...
| --------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **TDD development workflow** (`uniterra-plan` → `uniterra-implement` → `uniterra-simplify` / `uniterra-review`) | Plan → clarify requirements + design interactively → write `prd.md` / `design.md` / `acceptance.md` → review them with 3 parallel agents (feasibility / over-engineering / verifiable acceptance) → implement: write ALL failing property tests, decompose into a task list, run a batched / full-parallel workflow of subagents → review (correctness + security, findings pinned as failing regression tests written per repo conventions before fixing) and/or simplify (over-engineering checklist, behaviour-preserving; the plan's design is an authoritative constraint). Property testing blocks the bugs a known invariant would introduce, during development |
