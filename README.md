...
基於 DeepSeek Harness（dsh）agent runtime 與社群 dsh 插件構建的桌面 app：Electron shell 啟動內建 dsh CLI、把內建插件與技能 provision 進使用者 profile，並在視窗中託管 dsh Web UI。**目標是讓用戶通過插件快速構建自己的桌面 agent app** —— 內建 10 個 npm 社群插件、5 個 vendored 社群插件、1 個自家 provider 插件，用戶可隨時再加裝更多。
...
| 工作流                                         | 說明                                                                                                                                                                                                |
| ---------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **TDD 開發工作流**（`cardo-planmode`）         | 計畫 → 澄清 → PRD/設計 → 將業務邏輯定義為不變量並寫下會失敗的屬性測試（red phase）→ execution-plan.json → 執行 → 簡化/審查。整合屬性測試在開發期就把已知不變量會帶來的 bug 全部擋下                 |
...
| **項目文檔管理**（`project-documentation` 等） | 結構化 `docs/` 樹生成與增量更新；另有 QA 驗收（`qa`）、技能建立（`create-skill`）、AGENTS.md 管理（`manage-agents-md`）、git 流程（`manage-git-repo`）                                              |
