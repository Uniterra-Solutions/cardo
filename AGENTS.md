...
- `scripts/verify-windows-install/verify.ps1` — the Windows counterpart: replays the real `uniterra setup --source` on windows-latest (install → build → `--win --dir` packaging → embedded source → Start Menu shortcut → `Uniterra.exe` boot smoke to readiness). Runs in CI only (release gate), not on macOS dev machines.
